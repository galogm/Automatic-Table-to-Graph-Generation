...
from ..llm.bedrock import bedrock_llm_query, get_bedrock_llm
from ..llm.gconstruct import analyze_dataframes, extract_between_tags
from .action import (
    get_autog_actions,
...
class AutoG_Agent:
    ...
    def calculate_deepjoin(self, rdb_dataset):
        ...
        if (
            ...
            if os.path.exists(
                os.path.join(self.path_to_file, "round_0", f"deepjoin{self.round}.pkl")
            ):
                result = joblib.load(
                    os.path.join(self.path_to_file, "round_0", f"deepjoin{self.round}.pkl")
                )
                ...
                joblib.dump(
                    result, os.path.join(self.path_to_file, "round_0", f"deepjoin{self.round}.pkl")
                )
        else:
            logger.info("Calculate the deepjoin")
            model = load_pretrain_jtd_lm(self.lm_path)
    ...
    def decide_next_step(self, dbb, epoch):
        ...
        response = bedrock_llm_query(
            self.llm,
            this_round_prompt,
            max_tokens=self.output_size,
            cache=self.use_cache,
            debug_dataset=self.dataset,
            debug_task=self.task_name,
            debug_round=epoch - 1,
        )
    ...
    def augment(self):
        ...
        for i in range(self.threshold):
            ...
            if i == 0 and (
                not os.path.exists(os.path.join(self.dataset_cache_path, "data"))
                or len(os.listdir(os.path.join(self.dataset_cache_path, "data"))) == 0
            ):
                ...
                continue
            if i == 1:
                dbb = load_dbb_dataset_from_cfg_path_no_name(
                    os.path.join(self.path_to_file, "round_0")
                )
