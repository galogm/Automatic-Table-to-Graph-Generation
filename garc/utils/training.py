...
def train_val_test_split(
    indices, stratify=None, train_size=0.8, val_size=0.1, test_size=0.1, random_state=None
):
    """
...
def extract_train_val_test_id_from_objects(
    splits_file_path,
    target_column,
    original_table,
    train_ratio=0.6,
    ...
    if format == "custom":
        ...
        train_indices, val_indices, test_indices = train_val_test_split(
            indices, stratify, train_ratio, val_ratio, test_ratio
        )
        splits_dict = {"train": train_indices, "validation": val_indices, "test": test_indices}
