import os
from models.autog.action import (
    get_autog_actions,
...
from dbinfer_bench.dataset_meta import DBBColumnSchema, DBBColumnDType
...
def ordinal(n: int) -> str:
    ...
    suffix = (
        suffixes.get(n % 10, "th") if n % 10 in suffixes or n % 100 // 10 == 1 else "th"
    )
    return str(n) + suffix
...
class AutoM_Agent:
    ...
    def decide_next_step(self, dbb: DBBRDBDataset, llm_path):
        ...
        response = dummy_llm_interaction(
            this_round_prompt, query_file_path, response_file_path
        )
        ...
        for move in selection:
            ...
            try:
                ...
                move["parameters"] = {
                    k: v for k, v in move["parameters"].items() if k != "dbb"
                }
