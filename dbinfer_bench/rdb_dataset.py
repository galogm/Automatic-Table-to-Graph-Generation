...
class DBBRDBDataset:
    ...
    def _load_data(self):
        ...
        for task_meta in self.metadata.tasks:
            ...
            self._tasks.append(DBBRDBTask(task_meta, train_set, validation_set, test_set))
...
class DBBRDBTaskCreator:
    ...
    def done(
        self, path: Path, table_format: DBBTableDataFormat = DBBTableDataFormat.NUMPY
    ) -> DBBTaskMeta:
        path = Path(path)
        if not path.exists():
...
class DBBRDBDatasetCreator:
    ...
    def set_time_column(self, table: str, time_col: str):
        ...
        if other_time_col is not None and time_col != other_time_col:
            raise ValueError(
                f"A table can only have one time column but got {time_col} and {other_time_col}."
            )
        self.tables[table]["time_column"] = time_col

    def add_task(self, task_creator: DBBRDBTaskCreator):
        self.tasks.append(task_creator)
        return self

    def add_column_group(self, col_group: List[Tuple[str, str]]):
    ...
    def replace_tables_from(self, other: DBBRDBDataset):
        self.tables = {}
        for table_schema in other.metadata.tables:
            ...
            for col_schema in table_schema.columns:
                ...
                self.add_column(
                    table_name, col_name, other.tables[table_name][col_name], **col_schema
                )
        return self
    ...
    def done(self, path: Path, table_format: DBBTableDataFormat = DBBTableDataFormat.NUMPY):
        path = Path(path)
        if not path.exists():
            path.mkdir(parents=True, exist_ok=True)
        if not path.is_dir():
