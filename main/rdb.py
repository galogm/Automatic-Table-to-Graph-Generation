...
from dbinfer.cli import GMLSolutionChoice, construct_graph, fit_gml, preprocess
from garc.utils.data.rdb import load_dbb_dataset_from_cfg_path
from garc.utils.plot import plot_rdb_dataset_schema
from garc.utils.rdb import name_id_mapping
...
def main(
    ...
    method: str = typer.Argument("r2n", help="The method to use for graph construction"),
    model: str = typer.Argument("gat", help="The model to use for graph learning"),
    task_name: str = typer.Argument("venue", help="Name of the task to fit the solution."),
    num_of_seeds: int = typer.Argument(1, help="Number of seeds to use for training."),
    ...
    checkpoint_path: str = typer.Option(None, "--checkpoint_path", "-p", help="Checkpoint path."),
    use_wandb: bool = typer.Option(
        False, "--enable-wandb/--disable-wandb", help="Whether to use wandb for logging"
    ),
    plot_only: bool = typer.Option(
        False, "--plot-only", help="Whether to only plot the schema of the dataset"
    ),
    ...
    skip_train: bool = typer.Option(
        False, "--skip-train", help="Whether to skip the training process"
    ),
    use_cache: bool = typer.Option(
        False, "--use-cache", help="Whether to use the cache for the dataset"
    ),
):
    ...
    if use_cache and osp.exists(f"{cache_path}/{data_id}/{data_id}-preprocessed"):
        ...
        preprocessed_file_paths = preprocess(
            dataset,
            schema_path,
            "transform",
            f"{cache_path}/{data_id}/{data_id}-preprocessed",
            None,
        )
    ...
    if osp.exists(method):
        ...
        with open(method, "r") as f:
            ...
            if method == "Row2Node/Edge":
                ...
                method = "r2ne"
    if use_cache and osp.exists(
        f"{cache_path}/{data_id}/{data_id}-{config_file_name}-graph-{method}"
    ):
